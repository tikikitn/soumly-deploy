...
function TrustStrip() {
	return (
		<div className="sm-trust-strip">
			<span>
				<RefreshCw size={20} /> Prix du dernier relevé
			</span>
			<span>
				<Store size={20} /> Boutiques référencées
			</span>
			<span>
				<ShieldCheck size={20} /> Liens marchands directs
			</span>
		</div>
	);
}
...
function FilterPanel({
	...
	const ceiling = Math.max(
		100,
		Math.ceil(Math.max(...items.map((product) => product.price), 100) / 100) * 100,
	);
	...
	return (
		<aside className="sm-filter-panel">
			...
			<div className="sm-filter-group">
				<h3>Prix maximum</h3>
				<label className="sm-price-inputs">
					Jusqu’à{" "}
					<input
						value={maxPrice}
						min="0"
				...
				<input
					...
					min="0"
					max={ceiling}
					value={Math.min(maxPrice, ceiling)}
			...
			<div className="sm-filter-group">
				...
				{brands.map((brand) => (
					<label className="sm-check-row" key={brand}>
						...
						/>
						<span>{brand}</span>
						<small>{items.filter((product) => product.brand === brand).length}</small>
					</label>
				))}
			</div>
			<div className="sm-filter-group">
...
function Catalog({ baseProducts }: { baseProducts: Product[] }) {
	const initialCeiling = Math.max(
		100,
		Math.ceil(Math.max(...baseProducts.map((product) => product.price), 100) / 100) * 100,
	);
	...
	return (
		<section className="sm-page-shell sm-catalog-layout">
			...
			{filtersOpen ? (
				<div
					className="sm-filter-overlay"
					role="dialog"
					aria-modal="true"
					onClickCapture={() => setFiltersOpen(false)}
					onKeyDown={(event) => {
						if (event.key === "Escape") setFiltersOpen(false);
					}}
				>
					<div className="sm-filter-panel-wrap">
						<FilterPanel {...panelProps} close={() => setFiltersOpen(false)} />
					</div>
				</div>
			) : null}
			<div className="sm-catalog-results">
				<div className="sm-results-toolbar">
					...
					>
						<Filter size={18} /> Filtres{selectedBrands.length ? ` (${selectedBrands.length})` : ""}
					</button>
					<span>
				...
				{visibleProducts.length ? (
					<>
						...
						{displayedProducts.length < visibleProducts.length ? (
							<button
								className="sm-secondary-button sm-load-more"
								type="button"
								onClick={() => setDisplayLimit((limit) => limit + 60)}
							>
								Afficher plus ({visibleProducts.length - displayedProducts.length} restants)
							</button>
						) : null}
					</>
				) : (
		...
		<>
			<section className="sm-page-shell sm-inner-hero sm-categories-hero">
				<div>
					...
					<p>
						{products.length.toLocaleString("fr-FR")} produits classés en {families.length}{" "}
						familles.
					</p>
				</div>
				<div className="sm-hero-stat-grid">
					<div>
						<strong>{products.length.toLocaleString("fr-FR")}</strong>
...
export function FamilyScreen() {
	...
	if (!family)
		return (
			<EmptyState
				...
				text="Cette famille n’existe pas."
	...
	const groupSlugs = new Set(
		groups.flatMap((group) => group.categories.map((category) => category.slug)),
	);
					...
					<div>
						...
						<div>
							...
							<p>
								...
								{new Intl.NumberFormat("fr-FR").format(familyProducts.length)} produits.
							</p>
						</div>
					</div>
					<span className="sm-result-pill">{familyProducts.length} produits</span>
				</div>
			</section>
			<section className="sm-page-shell sm-section-block">
						...
						<div className="sm-group-card" key={group.name}>
							<div className="sm-group-card-head">
								{group.image ? (
									...
									<span className="sm-group-card-fallback">
										<SoumlyIcon name={family.icon} size={22} />
									</span>
								)}
								<h2>{group.name}</h2>
							</div>
							...
							<Link
								className="sm-group-more"
								href={`/categories/${group.categories[0]?.slug ?? ""}`}
							>
								Voir plus →
							</Link>
						</div>
		...
		<>
			<section className="sm-page-shell sm-listing-intro">
				...
				<div className="sm-listing-title-row">
					...
					<span className="sm-result-pill">{categoryProducts.length} produits</span>
				</div>
			</section>
...
export function SearchScreen() {
	...
	const matches = useMemo(() => {
		...
		return products.filter((product) => {
			const haystack = `${product.name} ${product.brand} ${product.category}`.toLocaleLowerCase(
				"fr",
			);
			const matchesText = words.length === 0 || words.every((word) => haystack.includes(word));
			const matchesMerchant =
				!merchant ||
				product.offers.some((offer) => offer.store.toLowerCase() === merchant.toLowerCase());
		...
		<>
			<section className="sm-search-hero">
				<div className="sm-page-shell">
					...
					<h1>{title}</h1>
					<p>
						{matches.length} produit{matches.length > 1 ? "s" : ""} correspondant
						{matches.length > 1 ? "s" : ""}.
					</p>
