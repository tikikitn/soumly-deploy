...
export default function SoumlyShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const update = () => {
      try {
        setFavoriteCount(JSON.parse(window.localStorage.getItem("soumly-favorites") ?? "[]").length);
      } catch {
        try {
          const cookie = document.cookie.split("; ").find((item) => item.startsWith("soumly-favorites="));
          setFavoriteCount(cookie ? JSON.parse(decodeURIComponent(cookie.split("=").slice(1).join("="))).length : 0);
        } catch {
          setFavoriteCount(0);
        }
      }
    ...
    <div className="soumly-pages">
      <header className="sm-site-header">
        <div className="sm-page-shell sm-header-row">
          <Link className="sm-logo" href="/" aria-label="Soumly, accueil">Soumly<span>.</span></Link>
          <nav className="sm-desktop-nav" aria-label="Navigation principale">
            {navItems.map((item) => (
              <Link key={item.href} className={pathname.startsWith(item.href) ? "is-active" : ""} href={item.href}>
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="sm-header-actions">
            <Link className={`sm-icon-button sm-favorite-header ${pathname === "/favoris" ? "is-active" : ""}`} href="/favoris" aria-label="Favoris">
              <Heart size={22} />
              {favoriteCount ? <span>{favoriteCount}</span> : null}
            </Link>
            <Link className="sm-login-button" href="/compte"><UserRound size={19} /> <span>Mon espace</span></Link>
            <button className="sm-mobile-menu-button" type="button" onClick={() => setMenuOpen((open) => !open)} aria-label="Ouvrir le menu" aria-expanded={menuOpen}>
        ...
        <div className="sm-page-shell sm-mobile-search-wrap">
          <form className="sm-mobile-search" onSubmit={submitSearch} role="search">
            ...
            <button type="submit" aria-label="Lancer la recherche"><Search size={18} /></button>
          </form>
        </div>
        {menuOpen ? (
          <nav className="sm-mobile-drawer" aria-label="Menu mobile">
            {navItems.map((item) => {
              ...
              return <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)}><Icon size={19} /> {item.label}</Link>;
            })}
          </nav>
        ) : null}
      </header>
      ...
      <footer className="sm-footer">
        ...
        <div className="sm-page-shell sm-footer-bottom"><span>© 2026 Soumly. Tous droits réservés.</span><span>Prix du dernier relevé · À confirmer chez le marchand</span></div>
      </footer>

      <nav className="sm-bottom-nav" aria-label="Navigation mobile">
        <Link href="/" className={pathname === "/" ? "is-active" : ""}><Home size={21} /><span>Accueil</span></Link>
        <Link href="/categories" className={pathname.startsWith("/categories") ? "is-active" : ""}><Grid3X3 size={21} /><span>Catégories</span></Link>
        <Link href="/favoris" className={pathname === "/favoris" ? "is-active" : ""}><Heart size={21} /><span>Favoris</span></Link>
        <Link href="/compte" className={pathname === "/compte" ? "is-active" : ""}><UserRound size={21} /><span>Compte</span></Link>
      </nav>
    </div>
  );
}
