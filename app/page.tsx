...
const categories: Array<{ label: string; icon: IconType }> = categoryList.map((label) => ({
	...
	icon:
		label === "Smartphones"
			? Smartphone
			: label === "PC Portable" || label === "PC de Bureau" || label === "Ordinateur"
				? Laptop
				: label === "Casque & Écouteurs" || label === "Audio" || label === "TV"
					? MonitorPlay
					: label === "Électroménager" || label === "Climatiseur"
						? WashingMachine
						: label === "Gaming"
							? Gamepad2
							: label === "Maison"
	...
	return (
		<main id="accueil">
			<header className="site-header">
				<div className="header-main page-shell">
					...
					<div className="header-actions">
						...
						<button className="login-button" type="button" onClick={() => router.push("/compte")}>
			...
			<section className="alert-section page-shell">
				...
				<button
					type="button"
					onClick={() => showToast("Votre future alerte est prête à être configurée")}
				>
				...
				<div className="page-shell footer-grid">
					<div className="footer-brand">
						...
						<span className="demo-note">
							Prototype visuel — les prix affichés sont des données de démonstration.
						</span>
					</div>
					<div>
						<h3>Soumly</h3>
				...
				<div className="modal-overlay" role="presentation" onClick={() => setSelectedProduct(null)}>
					...
					>
						...
						<div className="merchant-list">
							{["Mytek", "Tunisianet", "Spacenet"].map((store, index) => {
								const price =
									...
									index * Math.max(12, Math.round(selectedProduct.price * 0.025));
								return (
									<article key={store}>
										...
										<button
											type="button"
											onClick={() => showToast(`Ouverture de l'offre ${store}`)}
										>
											Voir l&apos;offre
										</button>
									</article>
						...
						<p className="modal-disclaimer">
							Prix de démonstration. Vérifiez toujours le prix final et la disponibilité chez le
							marchand.
						</p>
					</section>
				</div>
